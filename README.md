...
成分股输入表和最终 CSV、报告属于数据产物，可按下面的命令放在仓库外的 `outputs/` 目录，避免将大文件和缓存写入 Git 历史。
...
如使用不同日期的成分股名单，应在最终报告中同时修改成分股时点口径。
...
选择东方财富而非同时混用多个平台，主要考虑：数据链路完整、股票标识一致、公告原文可追溯，以及接口响应便于落地为 SQLite/CSV。需要注意这些接口属于公开网页数据接口，并非稳定性承诺的正式数据服务；生产环境应加入限速、重试、缓存、失败清单和接口变更监控。

## 筛选与数据口径

1. 标题和分类仅用于高召回：同时匹配“减持/权益变动”等事件信号与“5%以上股东/控股股东/实际控制人/一致行动人”等主体信号。
2. 最终资格回到 PDF 正文核验，并识别 `□是 √否` 等模板中的否定语义。
3. `股/万股/亿股` 统一换算为股；`减持股数口径`区分计划上限、实际减持和未识别。
4. 仅提取与本次或累计减持明确绑定的比例，避免把减持后剩余持股比例误作减持比例。
5. 相对交易日期暂保留为相对期间，不伪造绝对日期；金额 Top 20 只使用公告明确披露值。
6. 同一事件可能有计划、进展和结果等多份公告，因此公告数不能直接解释为独立减持事件数。

## 可复现性与版本历史

代码修改均通过独立 Git commit 留痕。项目压缩包如包含 `.git/`，解压后可使用以下命令核对完整历史：
...
数据仅用于数据工程与信息整理，不构成投资建议。东方财富页面结构或接口参数可能变化，重新运行前建议先用少量股票联调。
